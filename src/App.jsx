...
// Prompts diferenciados y directos por tema
const PROMPTS = {
  history: (place, extra='') =>
...
Empieza directo con los datos históricos.`,
  ...
  culture: (place, extra='') =>
    `Eres un experto en cultura y antropología. Habla de forma clara. Sin metáforas. Información concreta sobre la gente y sus costumbres.
Lugar: ${place}. ${extra}
Describe la cultura en 220-260 palabras:
- Costumbres y tradiciones más distintivas
- Fiestas o celebraciones importantes
- Arte, música o expresiones culturales propias
- Cómo es la vida cotidiana
Empieza directamente con la información cultural.`,
...
- Su importancia cultural hoy y por qué vale la pena visitarlo
...
export default function App() {
  ...
  const [inputMode, setInputMode] = useState('gps')   // gps | manual
  ...
  // Stories per tab
  const [stories,   setStories]   = useState({})   // { history:'...', culture:'...', ... }
  const [shown,     setShown]     = useState({})    // same shape, for typewriter
  ...
  const [audioCtx,  setAudioCtx]  = useState(null) // which story is playing: 'tab' | 'poi'
  ...
  // ── Typewriter ─────────────────────────────────────────
  function typewrite(text, setter, key) {
    ...
    typeTimers.current[key] = setInterval(() => {
      i++
      setter(prev => ({...prev, [key]:words.slice(0,i).join(' ')}))
      if (i >= words.length) clearInterval(typeTimers.current[key])
    }, 22)
  }

  function typewriteSingle(text, setter) {
    clearInterval(typeTimers.current['poi'])
    setter('')
    const words = text.split(' '); let i = 0
    typeTimers.current['poi'] = setInterval(() => {
      i++; setter(words.slice(0,i).join(' '))
      if (i >= words.length) clearInterval(typeTimers.current['poi'])
    }, 22)
  }
  ...
  function startAudio(text) {
    ...
    setPlaying(true);setProgress(0);next()
  }
  ...
  function toggleAudio(text, ctx) {
    ...
    } else if (synth.paused && audioCtx===ctx) {
      synth.resume()
  ...
  // ── Groq: generate one tab's content ──────────────────
  async function generateTab(tab, place, extra='') {
    if (stories[tab]) return // already generated
    setStories(p=>({...p,[tab]:'loading'}))
    try {
      const promptFn = PROMPTS[tab]||PROMPTS.history
      const text = await askGroq(promptFn(place, extra), groqKey, lang)
      setStories(p=>({...p,[tab]:text}))
      typewrite(text, setShown, tab)
    } catch {
  ...
  async function detectGPS() {
    ...
    setErr(''); setStories({}); setShown({}); setPois([]); setSelPoi(null); setPoiStory(''); setPoiShown(''); stopAudio()
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      ...
      // Generate first tab automatically
  ...
  async function searchManual() {
    ...
    setErr(''); setStories({}); setShown({}); setPois([]); setSelPoi(null); setPoiStory(''); setPoiShown(''); stopAudio(); setBusy(true)
    ...
    if (geo.lat && geo.lon) {
      ...
      try { const p=await fetchNearbyPOI(geo.lat,geo.lon); setPois(p) } catch {}
    }
    await generateTab('history', geo.full)
    setBusy(false)
  }
  ...
  async function switchTab(tab) {
    setActiveTab(tab)
    stopAudio()
    if (tab !== 'places' && geoInfo && !stories[tab]) {
  ...
  async function selectPoi(poi) {
    ...
    try {
      const prompt = PROMPTS.monument(name, poiType(poi), geoInfo?.full||'')
      const text = await askGroq(prompt, groqKey, lang)
      setPoiStory(text); typewriteSingle(text, setPoiShown)
        ...
        {!keySaved ? (
          <Card label="🔑 API Key de Groq">
            <p style={{fontFamily:'sans-serif',fontSize:'.73rem',color:'rgba(245,239,224,.65)',lineHeight:1.65,marginBottom:'.85rem'}}>
              Consigue tu key gratuita (sin tarjeta) en <strong style={{color:GL}}>console.groq.com</strong> → API Keys
            </p>
            <div style={{display:'flex',gap:'.5rem'}}>
              <input type="password" placeholder="gsk_..."
                value={keyInput} onChange={e=>setKeyInput(e.target.value)}
                onKeyDown={e=>e.key==='Enter'&&keyInput.trim()&&(setGroqKey(keyInput.trim()),setKeySaved(true))}
              ...
              />
              <button onClick={()=>{if(keyInput.trim()){setGroqKey(keyInput.trim());setKeySaved(true)}}}
          ...
          <>
            ...
            {geoInfo && (
              <div style={{display:'flex',alignItems:'center',gap:'.75rem'}}>
                ...
                <select value={lang} onChange={e=>{ setLang(e.target.value); setStories({}); setShown({}); }}
            ...
            {/* Content tabs — only show when we have a location */}
            {geoInfo && (
              <>
                {/* Tab bar */}
                <div style={{display:'flex',gap:'.3rem',overflowX:'auto',paddingBottom:'.1rem'}}>
                  {TABS.map(t=>(
                    <button key={t.key} onClick={()=>switchTab(t.key)}
                      style={{flexShrink:0,padding:'.5rem .7rem',background:activeTab===t.key?'rgba(200,150,62,.15)':'rgba(245,239,224,.04)',border:`1px solid ${activeTab===t.key?'rgba(200,150,62,.5)':'rgba(245,239,224,.1)'}`,borderRadius:9,color:activeTab===t.key?GL:'rgba(245,239,224,.4)',fontFamily:'sans-serif',fontSize:'.62rem',fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:'.3rem',whiteSpace:'nowrap'}}>
                      {t.icon} {t.label}
                    </button>
                  ))}
                </div>

                {/* ── Content tabs: History / Culture / Legends / Food ── */}
                {activeTab !== 'places' && (
                  <Card label={`${TABS.find(t=>t.key===activeTab)?.icon} ${TABS.find(t=>t.key===activeTab)?.label} — ${place}`} highlight={curDone}>
                    {stories[activeTab]==='loading' || (!stories[activeTab] && busy) ? (
                      ...
                      <Pill type="error">Error al generar. Intenta de nuevo.</Pill>
                    ) : (
                      <>
                        ...
                        {curDone && (
                          <>
                            ...
                            <div style={{display:'flex',alignItems:'center',gap:'.9rem'}}>
                              <button onClick={()=>toggleAudio(curStory,'tab')}
                                style={{width:52,height:52,borderRadius:'50%',background:`linear-gradient(135deg,${G},${T})`,border:'none',cursor:'pointer',fontSize:'1.2rem',flexShrink:0,boxShadow:'0 4px 16px rgba(200,150,62,.4)',display:'flex',alignItems:'center',justifyContent:'center',animation:playing&&audioCtx==='tab'?'wtpulse 2s infinite':''}}>
                                {playing&&audioCtx==='tab'?'⏸️':'▶️'}
                              </button>
                              <div style={{flex:1}}>
                                <div style={{height:4,background:'rgba(200,150,62,.15)',borderRadius:2,overflow:'hidden',marginBottom:'.35rem'}}>
                                  <div style={{height:'100%',background:`linear-gradient(90deg,${G},${GL})`,width:audioCtx==='tab'?(progress*100)+'%':'0%',transition:'width .4s linear'}}/>
                                </div>
                                <div style={{display:'flex',justifyContent:'space-between',fontFamily:'sans-serif',fontSize:'.6rem',color:'rgba(245,239,224,.3)'}}>
                                  <span>{audioCtx==='tab'?Math.round(progress*100):0}%</span>
                                  <span style={{color:playing&&audioCtx==='tab'?'#a0d0a0':audioCtx==='tab'&&progress>0?'#d0c080':'rgba(245,239,224,.3)'}}>
                                    {playing&&audioCtx==='tab'?'🔊 reproduciendo':audioCtx==='tab'&&progress>0?'⏸ pausado':'toca ▶ para escuchar'}
                                  </span>
                                </div>
                              </div>
                              <div style={{display:'flex',gap:3,alignItems:'flex-end',height:26}}>
                                {[7,15,10,21,13,8].map((h,i)=>(
                                  <div key={i} style={{width:3,background:G,borderRadius:2,height:playing&&audioCtx==='tab'?h:3,opacity:playing&&audioCtx==='tab'?.85:.2,transition:`height ${.2+i*.04}s ease,opacity .3s`}}/>
                                ))}
                              </div>
                            </div>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'.7rem'}}>
                              <button onClick={()=>{const o=[0.75,1.0,1.25,1.5];setSpd(o[(o.indexOf(spd)+1)%o.length]);if(playing){stopAudio();setTimeout(()=>startAudio(curStory),100)}}}
                              ...
                              <button onClick={()=>{stopAudio();setProgress(0);setTimeout(()=>{setAudioCtx('tab');startAudio(curStory)},150)}}
                ...
                {activeTab === 'places' && (
                  <>
                    {pois.length === 0 ? (
                      <Card>
                        <div style={{fontFamily:'sans-serif',fontSize:'.78rem',color:'rgba(245,239,224,.5)',textAlign:'center',padding:'1rem 0'}}>
                          {coords ? 'No se encontraron monumentos cercanos.' : 'Usa el GPS para ver monumentos cercanos.'}
                        </div>
                      </Card>
                    ) : (
                      <Card label={`🗺️ ${pois.length} monumentos y lugares cercanos`}>
                          ...
                          <>
                            ...
                            {poiDone && (
                              <>
                                <div style={{height:1,background:'rgba(200,150,62,.15)',marginBottom:'1rem'}}/>
                                <div style={{display:'flex',alignItems:'center',gap:'.9rem'}}>
                                  <button onClick={()=>toggleAudio(poiStory,'poi')}
                                    style={{width:52,height:52,borderRadius:'50%',background:`linear-gradient(135deg,${G},${T})`,border:'none',cursor:'pointer',fontSize:'1.2rem',flexShrink:0,boxShadow:'0 4px 16px rgba(200,150,62,.4)',display:'flex',alignItems:'center',justifyContent:'center',animation:playing&&audioCtx==='poi'?'wtpulse 2s infinite':''}}>
                                    {playing&&audioCtx==='poi'?'⏸️':'▶️'}
                                  </button>
                                  <div style={{flex:1}}>
                                    <div style={{height:4,background:'rgba(200,150,62,.15)',borderRadius:2,overflow:'hidden',marginBottom:'.35rem'}}>
                                      <div style={{height:'100%',background:`linear-gradient(90deg,${G},${GL})`,width:audioCtx==='poi'?(progress*100)+'%':'0%',transition:'width .4s linear'}}/>
                                    </div>
                                    <div style={{display:'flex',justifyContent:'space-between',fontFamily:'sans-serif',fontSize:'.6rem',color:'rgba(245,239,224,.3)'}}>
                                      <span>{audioCtx==='poi'?Math.round(progress*100):0}%</span>
                                      <span style={{color:playing&&audioCtx==='poi'?'#a0d0a0':audioCtx==='poi'&&progress>0?'#d0c080':'rgba(245,239,224,.3)'}}>
                                        {playing&&audioCtx==='poi'?'🔊 reproduciendo':audioCtx==='poi'&&progress>0?'⏸ pausado':'toca ▶ para escuchar'}
                                      </span>
                                    </div>
                                  </div>
                                  <div style={{display:'flex',gap:3,alignItems:'flex-end',height:26}}>
                                    {[7,15,10,21,13,8].map((h,i)=>(
                                      <div key={i} style={{width:3,background:G,borderRadius:2,height:playing&&audioCtx==='poi'?h:3,opacity:playing&&audioCtx==='poi'?.85:.2,transition:`height ${.2+i*.04}s ease,opacity .3s`}}/>
                                    ))}
                                  </div>
                                </div>
                                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'.7rem'}}>
                                  <button onClick={()=>{const o=[0.75,1.0,1.25,1.5];setSpd(o[(o.indexOf(spd)+1)%o.length]);if(playing){stopAudio();setTimeout(()=>startAudio(poiStory),100)}}}
                                    style={{fontFamily:'sans-serif',fontSize:'.62rem',fontWeight:700,background:'rgba(200,150,62,.1)',border:'1px solid rgba(200,150,62,.28)',color:G,padding:'.22rem .65rem',borderRadius:20,cursor:'pointer'}}>
                                    {spd}× velocidad
                                  </button>
                                  <button onClick={()=>{stopAudio();setProgress(0);setTimeout(()=>{setAudioCtx('poi');startAudio(poiStory)},150)}}
                                    style={{fontFamily:'sans-serif',fontSize:'.62rem',background:'none',border:'none',color:'rgba(245,239,224,.35)',cursor:'pointer'}}>
                                    ↺ Reiniciar
                                  </button>
                                </div>
            ...
            <button onClick={()=>{setKeySaved(false);setGroqKey('');setKeyInput('')}}
